...
export const renderCard = (card, hand, type) => {
  if (type === 'player') {
    document.getElementById('player-card-container').appendChild(createPlayerCard(card, hand));
  } else {
    document.getElementById('dealer-card-container').appendChild(createDealerCard(card, hand, type));
  }
};

const createPlayerCard = (card, hand) => {
...
const createDealerCard = (card, hand, type) => {
  ...
  const newCard = document.createElement('div');
  type === 'dealer-facedown' ? newCard.className = 'card flipped' : newCard.className = 'card';
