...
export const basicStrategy = (hand, dealerUpCard) => {
  dealerUpCard = parseInt(dealerUpCard.value());
  ...
  if (isSplittable(hand)) {
    if (hand.value === 4 || hand.value === 6) {
      ...
      return (dealerUpCard <= 7 || dealerUpCard === 9 ? 'split' : 'hit');
    }
    ...
    if (hand.value <= 11) {
      ...
      return (dealerUpCard === 3 || dealerUpCard === 2 ? 'hit' : 'stay');
